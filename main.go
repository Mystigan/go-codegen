...
func NewGen(publicName string) Gen {
	return Gen{
		Constructor: "New" + publicName,
		PrivateName: lowerFirst(publicName),
		PublicName:  publicName,
		ShortName:   lowerFirst(publicName)[:1],
	}
}
...
func main() {
	...
	// go-codegen ./hello.go -type=User -out=../model/user.go
	// go-codegen ./hello.go -type=User -out=../model
	...
	// Generate code.
	if err := generate(structName, structType, in, out); err != nil {
		log.Fatalf("generate failed: %s", err)
	}
}

func generate(structName string, structType *types.Struct, in, out string) error {
	entity := NewGen(structName)
		...
		case *types.Named:
			...
			case "sql":
				...
				case "NullTime":
					...
					let.If().Id(entity.ShortName).Dot(fieldPublic).Dot("Valid").Block(
						Id(fieldPrivate).Op("=").Id(entity.ShortName).Dot(fieldPublic).Dot(strings.Replace(name, "Null", "", -1)),
					)
				...
				default:
					...
					let.If().Id(entity.ShortName).Dot(fieldPublic).Dot("Valid").Block(
						Id(fieldPrivate).Op("=").Id(entity.ShortName).Dot(fieldPublic).Dot(underlyingType),
					)
					lets = append(lets, let)
					letsMapper[fieldPrivate] = true
				}
	...
	functionalOptionalFields := make([]Gen, len(fields))
	...
	for i := range fields {
		...
		if letsMapper[field.fieldPrivate] {
			...
			factory[i] = Id(entity.ShortName).Dot(field.fieldPublic)
		}
		functionalOptionalFields[i] = Gen{
			PrivateName: field.fieldPrivate,
			PublicName:  field.fieldPublic,
			ShortName:   lowerFirst(field.fieldPrivate)[:1],
	...
	for i := range fields {
		field := fields[i]
		f.Func().Params(Id(entity.ShortName).Id(entity.PublicName)).Id(field.fieldPublic).
			Params().
			Id(field.fieldType).Block(
			Return(Id(entity.ShortName).Dot(field.fieldPrivate)),
		).Line()
	}

	generateFunctionOptionalBuilder(f, structName, functionalOptionalFields)
...
func generateFunctionOptionalBuilder(f *jen.File, entityName string, fields []Gen) {
	entity := Gen{
		Constructor: "New" + entityName,
		PrivateName: lowerFirst(entityName),
		PublicName:  entityName,
		ShortName:   lowerFirst(entityName)[:1],
	}
	builder := Gen{
	...
	validator := Gen{
		Constructor: entity.Constructor + "Validator",
		PrivateName: entity.PrivateName + "Validator",
		PublicName:  entity.PublicName + "Validator",
		ShortName:   "v",
	}
	builderOption := Gen{
	...
	f.Type().Id(validator.PrivateName).Interface(
		Id("Validate").Params(Id(entity.PrivateName).Op("*").Id(entity.PublicName)).Error(),
	)

	// Define the EntityBuilder.
	f.Type().Id(builder.PublicName).Struct(
		Id(entity.PrivateName).Op("*").Id(entity.PublicName),
	)

	f.Func().Params(
		Id(builder.ShortName).Op("*").Id(builder.PublicName),
	).Id("Build").Params(Id(validator.ShortName).Id(validator.PrivateName)).Parens(List(Op("*").Id(entity.PublicName), Error())).Block(
		If(
			Err().Op(":=").Id(validator.ShortName).Dot("Validate").Call(Id(builder.ShortName).Dot(entity.PrivateName)),
			Err().Op("!=").Nil(),
		).Block(Return(List(Nil(), Err()))),
		Id(entity.ShortName).Op(":=").Id(builder.ShortName).Dot(entity.PrivateName),
		Id(builder.ShortName).Dot(entity.PrivateName).Op("=").Nil(),
		Return(List(Id(entity.ShortName), Nil())),
	)
	...
	f.Func().Id(builder.Constructor).Params(Id("opts").Op("...").Id(builderOption.PublicName)).Parens(Op("*").List(Id(builder.PublicName), Error())).Block(
		Id(builder.ShortName).Op(":=").Id(builder.PublicName).Values(Dict{
			Id(entity.PrivateName): Id("new").Parens(Id(entity.PublicName)),
		}),
	...
	for _, field := range fields {
		name := "With" + entity.PublicName + field.PublicName
		f.Func().Id(name).Params(Id(field.PrivateName).Id(field.Kind)).Id(builderOption.PublicName).Block(
			Return(Func().Params(Id(entity.ShortName).Op("*").Id(builder.PublicName)).Error().Block(
				Id(entity.ShortName).Dot(entity.PrivateName).Dot(field.PrivateName).Op("=").Id(field.PrivateName),
				Return(Nil()),
			)),
		)
	}
